...
from createTrainingData import EDC_coking
...
RESULTPATH = os.path.join(dir_path,'results','coking',NAME)
...
def arg_parser():
    ...
    parser.add_argument('--no-chcl3',action='store_false',help='don\'t generalization of CHCl3')
    parser.add_argument('--no-tri',action='store_false',help='don\'t generalization of Tri')
    parser.add_argument('--no-CP',action='store_false',help='don\'t generalization of CP')
...
def DataPreprocessor():
    ...
    if DOKFOLD:
        ...
        X_train = np.ravel(Xi_train).reshape(len(y_train),38)
        X_test = np.ravel(Xi_test).reshape(len(y_test),38)
        # # Shuffle the lists
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, CHCl3_X_0,Tri_X_0,CP_X_0, mass_flow_rate, 
    ...
    sp_lst = [CCl4_X_0,CHCl3_X_0,Tri_X_0,CP_X_0]
    for sp in sp_lst:
        if sp > 1 :  #ppm
            sp = float(sp) / 1000000
    ...
    if save_fig:
        ...
        for label in results.keys():
            cracking_rates = [i * 100 for i in results[label]['coking_rates']]
            lns += ax2.plot(range(ndata), cracking_rates, marker='o', label=label)   
        ax2.set_ylabel('Coking rates (%)')
        ax2.set_ylim(-5, 100)
        ...
        if DOKFOLD:
            ...
            plt.savefig(os.path.join(PLOTPATH, f'predict/CCl4_{CCl4_X_0:.6f}_CHCl3_{CHCl3_X_0:6f}_Tri_{Tri_X_0:6f}_CP_{CP_X_0}__mass_{mass_flow_rate}_temp_{T_list[0]}_{name}.png'))
        else:
...
def generalization_test():
    ...
    reaction_mech = {
        'Schirmeister': '../KM/2009_Schirmeister_EDC/chem_annotated_reversible.cti',
    ...
    if MASS_FLOW_RATE:
        ...
        plt.savefig(os.path.join(PLOTPATH, 'Generalization-Mass flow rate0.png'))
        FinalTime = time.time()
    ...
    if CCL4:
        ...
        for key,value in result_2.items():
            print(f"Error of CCl4 in concentration in {key}ppm is {value:.6f} ")
    ...
    if CHCL3:
        ...
        for key,value in result_3.items():
            print(f"Error of CHCl3 in concentration in {key}ppm is {value:.6f} ")
        # for i, CCl4 in enumerate(CCl4_X_0_list):
        #     loss = predict(reaction_mech, T_list, pressure_0, CCl4, mass_flow_rate,
        #                    n_steps, n_pfr, length, area)
        #     # print('Index:{:<4d} Loss:{:<8.5f}'.format(i+1, loss))
        ...
        plt.savefig(os.path.join(PLOTPATH, 'Generalization-Initial inlet CHCl3 concentration.png'))
        FinalTime = time.time()
        ComputationTime = FinalTime-InitialTime
        print('CHCl3 concentration generalization takes {} min {} sec.'.format(ComputationTime/60,ComputationTime%60))
    ...
    if TRI:
        ...
        for key,value in result_4.items():
            print(f"Error of Tri in concentration in {key}ppm is {value:.6f} ")
        # for i, CCl4 in enumerate(CCl4_X_0_list):
        #     loss = predict(reaction_mech, T_list, pressure_0, CCl4, mass_flow_rate,
        #                    n_steps, n_pfr, length, area)
        #     # print('Index:{:<4d} Loss:{:<8.5f}'.format(i+1, loss))
        ...
        plt.savefig(os.path.join(PLOTPATH, 'Generalization-Initial inlet Tri concentration.png'))
        FinalTime = time.time()
        ComputationTime = FinalTime-InitialTime
        print('Tri concentration generalization takes {} min {} sec.'.format(ComputationTime/60,ComputationTime%60))
    ...
    if CP:
        ...
        for key,value in result_5.items():
            print(f"Error of CP in concentration in {key}ppm is {value:.6f} ")
        # for i, CCl4 in enumerate(CCl4_X_0_list):
        #     loss = predict(reaction_mech, T_list, pressure_0, CCl4, mass_flow_rate,
        #                    n_steps, n_pfr, length, area)
        #     # print('Index:{:<4d} Loss:{:<8.5f}'.format(i+1, loss))
        ...
        plt.savefig(os.path.join(PLOTPATH, 'Generalization-Initial inlet CP concentration.png'))
        FinalTime = time.time()
        ComputationTime = FinalTime-InitialTime
        print('CP concentration generalization takes {} min {} sec.'.format(ComputationTime/60,ComputationTime%60))
    
    # Case 6
    if INITAL_PRESSURE:
        InitialTime = time.time()
        print('Case6: Initial pressure...')
    ...
    if T_IN:
        ...
        T_biglist = [
            ...
            [310,340,368.35,393.65,416.5,441.6,452,458.3,464.6,471,474,476.5,478.8,481.1,482.1,483.1,484.1,485.1,486.7],
...
if __name__ == '__main__':
    ...
    if DOKFOLD:
        ...
        if PREDICT:
            ...
            # T_list = [320,348,374.7,399.3,424,451.3,460.7,466,471.3,476.7,478.7,479.3,480,481.3,482.3,483.3,484.3,485.3,486.7]
            T_list = [322,350,375,399,424,451,461,466,471,477,479,479,480,481,482,483,484,485,486,486,486,486,486]
            ...
            n_pfr = 22
            length = 18
            area = 3.14 * ((11.1 * 2.54 - 2 * 0.818)/ 100) ** 2 / 4 
            # Prediction
            ...
            for CCl4_X_0 in np.linspace(0, 2500, 6):
                loss = predict(reaction_mech, T_list, pressure_0, CCl4_X_0, CHCl3_X_0,Tri_X_0,CP_X_0, mass_flow_rate, 
                n_steps, n_pfr, length, area, save_fig=False, name=NAME,iter_CCl4=True)
