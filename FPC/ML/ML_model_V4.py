...
def arg_parser():
    ...
    parser.add_argument('--no_mass',action='store_false',help='don\'t generalization  of mass flow rate')
    parser.add_argument('--no_ccl4',action='store_false',help='don\'t generalization of CCl4')
    parser.add_argument('--no_pressure',action='store_false',help='don\'t generalization of initial pressure')
    parser.add_argument('--no_tubes',action='store_false',help='don\'t generalization of number of tubes')
    ...
    parser.add_argument('--no_predict',action="store_false",help='no prediction')
    parser.add_argument('--cpu',default=8,type=int,help='ncpus')
    parser.add_argument('--no_train',action='store_false',help='no training')
    parser.add_argument('--no_gene',action='store_false',help='no generalization')
    parser.add_argument('--no_T_in',action='store_false',help='no T_in')
    parser.add_argument('--FPC',action='store_true',help='do everything with FPC data')
    return parser
...
def build_model(lr=0.001):
    first_input = Input(shape=(6,), name='Input_layer_1')
    second_input = Input(shape=(29,), name='Input_layer_2')
    
    layer = Dense(40, name='Hinden_layer_1')(first_input)
    layer = Activation('relu')(layer)
    layer = Dense(20, name='Hinden_layer_2')(layer)
    layer = Activation('relu')(layer)
    layer = Dense(10, name='Hinden_layer_3')(layer)
    
    layer = concatenate([layer, second_input], name='Concatenate_layer')
    layer = Activation('relu')(layer)
    layer = Dense(20, name='Hinden_layer_4')(layer)
    layer = Activation('relu')(layer)
    layer = Dense(10, name='Hinden_layer_5')(layer)
    layer = Activation('relu')(layer)
...
#     layer = Dense(10, name='Hinden_layer_1')(first_input)
#     layer = Activation('relu')(layer)
#     layer = Dense(8, name='Hinden_layer_2')(layer)
#     layer = Activation('relu')(layer)
#     layer = Dense(5, name='Hinden_layer_3')(layer)
    
#     layer = concatenate([layer, second_input], name='Concatenate_layer')
#     layer = Activation('relu')(layer)
#     layer = Dense(15, name='Hinden_layer_4')(layer)
#     layer = Activation('relu')(layer)
#     layer = Dense(10, name='Hinden_layer_5')(layer)
#     layer = Activation('relu')(layer)
#     layer = Dense(1, name='Hinden_layer_6')(layer)
...
def DataPreprocessor():
    ...
    if DOKFOLD:
        ...
        X_train = [np.concatenate((rescaled_X_train[:,0:2],rescaled_X_train[:,-4:]),axis=1), rescaled_X_train[:,2:-4]]
        X_test = [np.concatenate((rescaled_X_test[:,0:2],rescaled_X_test[:,-4:]),axis=1), rescaled_X_test[:,2:-4]]
        # X_train = rescaled_X_train
        # X_test = rescaled_X_test
...
def train_model(X_train=None,X_test=None,y_train=None,y_test=None,fold_no=None,verbose=1):
    ...
    # Early stopping
    early_stopping = callbacks.EarlyStopping(monitor='val_loss', mode='min', patience=200,restore_best_weights = True)
    ...
    if DOKFOLD:
        ...
        loss, acc,*is_anything_else_being_returned = model.evaluate(X_test , y_test, batch_size=batch_size,verbose=0)
        print(f'Testset loss for {fold_no}: {loss:.4f}')
        return loss, acc*100

    else:
        # plt.savefig(os.path.join(PLOTPATH,'Learning curve.png'))
        loss,acc,*is_anything_else_being_returned = model.evaluate(x=X_test , y=y_test, batch_size=batch_size)
        print(is_anything_else_being_returned)
        textstr=f'Testset loss : {loss:4f}'
        print(f'Testset loss/acc:{loss}')
        ax1.text(0.85,0.55,textstr, ha="right", va="top",transform=ax1.transAxes,bbox=dict(facecolor='wheat', alpha=0.5))
        plt.savefig(os.path.join(PLOTPATH,'Learning curve.png'))
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate, 
            n_steps, n_pfr, length, area, save_fig=False, name='predict',fold_no=None,iter_CCl4=False,FPC=True):
    """
    ...
    for i, T in enumerate(T_list[1:]):
        ...
        x_predict= [np.concatenate((rescaled_x_predict[:,0:2],rescaled_x_predict[:,-4:]),axis=1), rescaled_x_predict[:,2:-4]]
        # x_predict = rescaled_x_predict
    ...
    if save_fig:
        ...
        textstr = '\n'.join(
            ...
            r'Pin=%.2fatm' % (pressure_0),
            r'Tin=%d°C' % (T_list[0]),
            r'Mass=%dT' % (mass_flow_rate),
            r'scale=%d' % (scale))
            )
        ...
        if DOKFOLD:
            ...
            plt.savefig(os.path.join(PLOTPATH, 
                f'predict/{name}/CCl4_{CCl4_X_0:.6f}_mass_{mass_flow_rate}_temp_{T_list[0]}_{name}.png'))
            with open(os.path.join(PLOTPATH, f'predict/{name}/Temperature_profile.txt'),'w') as f:
                f.writelines(str(T_list))

        else:
            plt.savefig(os.path.join(PLOTPATH, '{}.png'.format(name)))
            with open(os.path.join(PLOTPATH, f'Temperature_profile_{name}.txt'),'w') as f:
                f.writelines(str(T_list))  
...
def generalization_test():
    ...
    if MASS_FLOW_RATE:
        ...
        # mass_flow_rate_list = [54,64,72,88,106]
        if FPC:
            mass_flow_rate_list = [27,32,36]
        else:
            mass_flow_rate = [54,64,72,80,105]
        result_1 = {}
        # multiprocessing
        ...
        def MFR_generalization(mfr):
            ...
            loss = predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mfr,
                        n_steps, n_pfr, length, area,FPC=FPC)
            tup = (mfr,loss)
            return tup
        result_1 = {}
    ...
    if INITAL_PRESSURE:
        ...
        plt.savefig(os.path.join(PLOTPATH, 'Generalization-Initial inlet pressure0.png'))
        FinalTime = time.time()
        ComputationTime = FinalTime-InitialTime
    ...
    if NUMBER_OF_TUBES:
        ...
        plt.xticks(range(16, 23))
        plt.savefig(os.path.join(PLOTPATH, 'Generalization-Number of tubes0.png'))
    ...
    if T_IN:
        ...
        T_biglist = [
            [300,332,361,387,411,429.6,443.4,453.7,460.5,464.9,468.7,473,477.8,480.5,481.5,482.5,483.5,484.5,486.7],
            [310,340,368.4,393.7,416.5,441.6,452,458.3,464.6,471,474,476.5,478.8,481.1,482.1,483.1,484.1,485.1,486.7],
            [320,348,374.7,399.3,424,451.3,460.7,466,471.3,476.7,478.7,479.3,480,481.3,482.3,483.3,484.3,485.3,486.7],
            [330,357.3,382,406.6,431.3,455.1,461.5,467.8,473.1,476.8,478.8,479.4,480.1,481.4,482.4,483.4,484.4,485.4,486.7],
            [340,365.3,390,414.6,439.3,458.6,464,469.3,474.6,478,479,479.6,480.3,481.6,482.6,483.6,484.6,485.6,486.7],
            [350,374.7,399.3,424,449.3,460.7,466,471.3,476.7,478.7,479.3,480,481.3,482.3,483.3,484.3,485.3,486,486.7]
        ]
        result_5 = {}
        # multiprocessing
        ...
        plt.savefig(os.path.join(PLOTPATH, 'Generalization-Initial inlet temperature.png'))
        FinalTime = time.time()
        ComputationTime = FinalTime-InitialTime
        print('Initial temperature generalization takes {} min {} sec.'.format(ComputationTime/60,ComputationTime%60))
...
if __name__ == '__main__':
    ...
    if DOKFOLD:
        ...
        for train,test in kfold.split(Xi,yi):
            ...
            # Scale testing data
            rescaled_X_test = scaler.transform(X_test)
            new_X_train = [np.concatenate((rescaled_X_train[:,0:2],rescaled_X_train[:,-4:]),axis=1), rescaled_X_train[:,2:-4]]
            new_X_test = [np.concatenate((rescaled_X_test[:,0:2],rescaled_X_test[:,-4:]),axis=1), rescaled_X_test[:,2:-4]]
        ...
        if PREDICT:
            ##20##
            # T_list = [310, 340, 360, 386, 430, 460, 464, 467, 469, 470, 472, 473, 475, 477, 479, 482, 484, 485, 486]
            # T_list = [350, 380, 396, 413, 428, 436, 444, 453, 459, 463, 465, 467, 469, 470, 471, 472, 473, 474, 475]
            # T_list = [320,350,375,399,424,440,445,450,452,456,460,462,464,467,470,474,475,476,477]
            # T_list = [320,348,374.7,399.3,424,451.3,460.7,466,471.3,476.7,478.7,479.3,480,481.3,482.3,483.3,484.3,485.3,486.7]
            ...
            # T_list = [300,332,361,387,411,429.6,443.4,453.7,455.1,457.5,460.5,464.9,466.8,468.7,473,475.4,477.8,480.5,481.5,482.5,483.5,484.5,486.7]
            # T_list =[350,374.7,399.3,424,449.3,460.7,466,468.5,471.3,473.5,475.8,476.7,477.7,478.7,479.3,480,481.3,482.3,483.3,484.3,485.3,486,486.7]
            # T_list =[350,374.7,399.3,424,440.3,443.7,446,448.5,451.3,453.5,455.8,456.7,457.7,458.7,459.3,460,461.3,462.3,463.3,464.3,465.3,466,466.7]
            # T_list =[350,374.7,399.3,424,440.3,450.7,456,458.5,461.3,463.5,465.8,466.7,467.7,468.7,469.3,470,471.3,472.3,473.3,474.3,475.3,476,476.7]
            T_list = [320,350,375,399,424,440,445,450,452,454,456,458,460,461,462,464,467,470,472,474,476,476,477]
            # T_list = [322,350,375,399,424,451,455,460,464,467,469,470,474,476,477,478,479,480,481,483,484,485,486]
            # T_list = [350,374.7,399.3,424,451.3,460.7,466,471.3,476.7,478.7,479.3,479.4,479.5,479.6,479.7,479.8,479.9,480,480.2,480.4,480.6,480.8,482]
            # T_list = [300,340,375,399,424,451,455,460,463,465,468,471,473,475,477,479,480,481,482,483,484,485,486]
            ...
            # pool = mp.Pool(CPU)
            # CCl4_X_0_list = np.linspace(0, 2500, 21)
            # global prediction_mp
            # def prediction_mp(CCl4):
            #     predict(reaction_mech, T_list, pressure_0, CCl4, mass_flow_rate,
            #             n_steps, n_pfr, length,area=area,save_fig=True,name=name,iter_CCl4=True,FPC=True)
            # pool.map(prediction_mp,list(CCl4_X_0_list)) 
            # pool.close()
            # pool.join()           
