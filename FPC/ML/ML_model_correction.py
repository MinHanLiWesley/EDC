...
def arg_parser():
    ...
    parser.add_argument('--data',default="training_data_correction_10_10_10_5.csv",help='Training data')
...
def build_model(lr=0.001,Schirmeister_X = None):
    first_input = Input(shape=(2,), name='Input_layer_1')
    second_input = Input(shape=(34,), name='Input_layer_2')
...
def DataPreprocessor():
    ...
    if DOKFOLD:
        ...
        with open(os.path.join(RESULTPATH,'clf.pickle'), 'wb') as f:
            pickle.dump(scaler, f)
        # Change the shape of X in order to separate to ordinary input (in-out temp) and auxilary input (29-species mass fraction and  pressure in ,CCL4 initial ratio,t,tr,scim_x for every tube)
...
def train_model(X_train=None,X_test=None,y_train=None,y_test=None,fold_no=None,verbose=0):
    ...
    print('Bulid model...')
    Schirmeister_X = 0.01
    model = build_model(lr,Schirmeister_X=Schirmeister_X)
    # model.summary()
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate, 
    ...
    for i, T in enumerate(T_list[1:]):
        ...
        schi_X = results[KM_label]['cracking_rates'][i] 
        x_predict = [Ti, Te, compositions, pressure_0, CCl4_X_0, t, t_r, schi_X]
    ...
    results['ML'] = {'correction_factor': y_predicted}
    from sklearn.metrics import mean_absolute_error


    products = [a * b for a, b in zip(results['ML']['correction_factor'], results['Schirmeister']['cracking_rates'])]
    results['ML'] = {'cracking_rates':products}
    ...
    if save_fig:
        ...
        if DOKFOLD:
            ...
            plt.savefig(os.path.join(PLOTPATH, f'CCl4_{CCl4_X_0:.4f}_{name}.png'))
        else:
...
def generalization_test():
    ...
    if CCL4:
        ...
        plt.savefig(os.path.join(PLOTPATH, 'Generalization-Initial inlet CCl4 concentration2.png'))
        FinalTime = time.time()
        ComputationTime = FinalTime-InitialTime
...
if __name__ == '__main__':
    ...
    if DOKFOLD:
        ...
        for CCl4_X_0 in np.linspace(0, 0.0020, 20):
            loss = predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate,
