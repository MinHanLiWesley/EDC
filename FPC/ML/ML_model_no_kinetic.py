...
def build_model(lr=0.001):
    input = Input(shape=(6,), name='Input_layer_1')
    ...
    layer = Activation('relu')(layer)
    layer = Dense(15, name='Hinden_layer_5')(layer)
    layer = Activation('relu')(layer)
...
def DataPreprocessor():
    ...
    if DOKFOLD:
        ...
        X_train = np.concatenate((X_train[:, :2], X_train[:, -4:]), axis=1)
        X_test = np.concatenate((X_test[:, :2], X_test[:, -4:]), axis=1)
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate,
    ...
    for i, T in enumerate(T_list[1:]):
        ...
        x_predict = np.concatenate(
            (x_predict[:, :2], x_predict[:, -4:]), axis=1)
    ...
    if CCl4_X_0 < 1: 
        CCl4_X_0 = float(CCl4_X_0) * 1000000 # ppm
    ...
    if save_fig:
        ...
        if DOKFOLD:
            ...
            plt.savefig(os.path.join(
                PLOTPATH, f'predict/CCl4_{CCl4_X_0:.6f}_mass_{mass_flow_rate}_temp_{T_list[0]}_{name}.png'))
        else:
...
def generalization_test():
    ...
    T_list = [320, 348, 374.7, 399.3, 424, 451.3, 460.7, 466, 471.3, 476.7,
...
if __name__ == '__main__':
    ...
    if DOKFOLD:
        ...
        if PREDICT:
            ...
            import imageio
            import glob
            filenames = glob.glob(f"{PLOTPATH}/*_{name}.png")
            print(filenames)
            with imageio.get_writer(f'{PLOTPATH}/{name}.gif', mode='I') as writer:
