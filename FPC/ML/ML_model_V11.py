...
def arg_parser():
    ...
    parser.add_argument('--epoch', default=1500, type=int, help='epoch')
...
def DataPreprocessor():
    ...
    X = array[:-22, 0:-1]  # X.shape=(alot,36)
    Y = array[:-22, -1]  # Y.shpae=(alot,1)
    X_texas = array[-22:, :-1]
    y_texas = array[-22:, -1]
    print(X_texas.shape)
    if DOKFOLD:
        ...
        # Xi, Yi = [], []
        # for i in range((len(dataframe)-22)//18):
        ...
        # X_train = np.ravel(Xi_train).reshape(len(y_train), len(dataframe.columns)-1)
        # X_test = np.ravel(Xi_test).reshape(len(y_test), len(dataframe.columns)-1)
        ...
        rescaled_X_test = scaler.transform(X_test[:, :-1])
        rescaled_X_texas = scaler.transform(X_texas[:, :-1])
        ...
        # X_test = [rescaled_X_test[:, 0:2],
        #           rescaled_X_test[:, 2:], X_test[:, -1]]
        X_texas = [rescaled_X_texas[:, :2],
                   rescaled_X_texas[:, 2:], X_texas[:, -1]]
        return X_train, X_test, y_train, y_test, X_texas, y_texas
...
def train_model(X_train=None, X_test=None, y_train=None, y_test=None, X_texas=None, y_texas=None, fold_no=None, verbose=1):
    ...
    # hist = model.fit(X_train, y_train,
    #                 validation_data=(X_texas,y_texas),
    #                 batch_size=batch_size,
    #                 epochs=epoch,
    #                 callbacks=[early_stopping, csvLogger_callback],
    #                 verbose=verbose,
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate,
    ...
    KM_label = 'CANSPEN'
    print(results[KM_label]['cracking_rates'])
    y_predicted = [0]
    ...
    if print_cracking:
        ...
        print(','.join(map(str,np.round([i*100 for i in y_predicted],2))))
    # if loss:
    ...
    # df = pd.read_csv("training_data_FPC_V12_aspenparams_Y.csv")
    # a_series = pd.DataFrame(texas_training,columns=df.columns)
    # df = df.append(a_series,ignore_index=True)
    # print(df.iloc[len(df)-1])
    # df.to_csv("training_data_FPC_V13_addtexas_aspenparams_Y.csv",index=False)
            ...
            if len(y_ground_df) >= 18:
                for index in y_ground_df.index:
                    try:
                        ...
                        continue
            print(y_ground)
            print(f"len of y_ground: {len(y_ground)}")
    ...
    #                                         }
    # results['Aspen'] = {'cracking_rates': [-0.00011951931095932622, 7.320275376976326e-05, 0.0022277134329118375, 0.03015456399965144, 0.16023337218652456, 0.5978028329013729, 1.8700004947963866, 4.989545110406235, 12.26596639044677, 20.91000150698261, 27.307761321969426, 31.218327665015167, 34.21836923694941, 36.817712942630955, 39.177736060241344, 41.366043592239876, 43.42819996009998, 45.39328247248176, 47.27575409568278, 49.088598829252994, 50.84243830239199, 52.54495677083675, 54.20715746703109]
    #                                         }
    # results['Aspen']['cracking_rates'] = [i/100 for i in results['Aspen']['cracking_rates']]
    if CCl4_X_0 < 1:  # ppm
        CCl4_X_0 = float(CCl4_X_0) * 1000000
...
def generalization_test(scale=20):
    ...
    reaction_mech = {
        ...
        'CANSPEN': '../KM/2009_Schirmeister_EDC/test.cti',
    ...
    if MASS_FLOW_RATE:
        ...
        def MFR_generalization(mfr):
            ...
            loss = predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mfr,
                           n_steps, n_pfr, length, area, loss=True, print_cracking=True)
            tup = (mfr, loss)
            return tup
        result_1 = {}
...
if __name__ == '__main__':
    ...
    if DOKFOLD:
        ...
        if TRAINING:
            X_train, X_test, y_train, y_test, X_texas, y_texas = DataPreprocessor()
            InitialTime = time.time()
            train_model(X_train, X_test, y_train, y_test, X_texas, y_texas)
            FinalTime = time.time()
            ComputationTime = FinalTime-InitialTime
            print('Training takes {} min {} sec.'.format(
                ComputationTime/60, ComputationTime % 60))
        ...
        if PREDICT:
            ...
            ##30##
            # T_list = [320, 348, 7374.7, 399.3, 424, 451.3, 460.7, 462, 466, 468, 471.3, 474.3,
            #           476.7, 477.7, 478., 479.3, 480, 481.3, 482.3, 483.3, 484.3, 485.3, 486.7]
            ...
            # T_list = [320., 340.66011621, 360.97830372, 380.47483938, 398.67,
            #           415.08406242, 429.23730348, 440.65, 449.00918472, 454.66891397,
            ...
            if single:
                ...
                for CCl4_X_0 in np.linspace(0, 2500, 21):
                    loss = predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate,
                                   n_steps, n_pfr, length, area, save_fig=False, name=name, iter_CCl4=True, FPC=FPC)
