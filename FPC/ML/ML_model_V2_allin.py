...
def DataPreprocessor():
    ...
    if DOKFOLD:
        ...
        #     Xi, Yi, test_size=0, random_state=42)
        y_train = np.ravel(Yi)
        X_train = np.ravel(Xi).reshape(len(y_train),35)
        ...
        new_X_train = [rescaled_X_train[:, 0:2], rescaled_X_train[:, 2:]]
    ...
    if T_IN:
        ...
        def T_in_generalization(T_list):
            ...
            loss = predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate,
                           n_steps, n_pfr, length, area,save_fig=True, name=T_list[0])
            tup = (T_list[0], loss)
            return tup
...
if __name__ == '__main__':
    ...
    if DOKFOLD:
        ...
        X_train,  y_train = DataPreprocessor()
        # Model training
