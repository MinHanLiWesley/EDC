...
#     layer = Dense(12, name='Hinden_layer_2')(layer)
#     layer = Activation('relu')(layer)
...
def build_model(lr=0.001):
    ...
    layer = Dense(12, name='Hinden_layer_1')(input)
    layer = Activation('relu')(layer)
    layer = Dense(15, name='Hinden_layer_2')(layer)
    layer = Activation('relu')(layer)
    layer = Dense(15, name='Hinden_layer_3')(layer)
    layer = Activation('relu')(layer)
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate,
            n_steps, n_pfr, length, area, save_fig=False, name='predict', fold_no=None, iter_CCl4=False, FPC=FPC):
    """
...
def generalization_test():
    ...
    # mass_flow_rate = 105
    if FPC:
        mass_flow_rate_list = 36
    else:
        mass_flow_rate_list = 72
    n_pfr = 18
    ...
    if MASS_FLOW_RATE:
        ...
        plt.savefig(os.path.join(
            PLOTPATH, 'Generalization-Mass flow rate.png'))
        FinalTime = time.time()
    ...
    if DOKFOLD:
        ...
        if PREDICT:
            ...
            if n_pfr == 18:
                ...
                area = 3.14 * ((262) / 1000) ** 2 / 4
            name = '30_350_11.4_53_h466'
            # Prediction
            print('Starting prediction...')
            loss = predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate,
                           n_steps, n_pfr, length, area, save_fig=True, name=NAME, FPC=FPC)
            # CCl4_X_0_list = np.linspace(0, 2500, 21)
            # result_2 = {}
