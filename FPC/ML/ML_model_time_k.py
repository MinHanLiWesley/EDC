...
def build_model(lr=0.001):
    first_input = Input(shape=(29,), name='Input_layer_1')
    second_input = Input(shape=(2,), name='Input_layer_2')
    ...
    layer = Activation('relu')(layer)
    layer = Dense(63, name='Hinden_layer_4')(layer)
    layer = Activation('relu')(layer)
    layer = Dense(53, name='Hinden_layer_5')(layer)
    layer = Activation('relu')(layer)
...
def DataPreprocessor():
    ...
    if DOKFOLD:
        ...
        print(X_train.shape)
        X_train = np.concatenate((X_train[:,2:-4],X_train[:,-2:]),axis=1)
        print(X_train.shape)
        X_test = np.concatenate((X_test[:,2:-4],X_test[:,-2:]),axis=1)
        ...
        X_train = [rescaled_X_train[:,:-2], rescaled_X_train[:,-2:]]
        X_test = [rescaled_X_test[:,:-2], rescaled_X_test[:,-2:]]
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate, 
    ...
    for i, T in enumerate(T_list[1:]):
        ...
        x_predict = np.concatenate((x_predict[:,2:-4],x_predict[:,-2:]),axis=1)
...
def generalization_test():
    ...
    if CCL4:
        ...
        CCl4_X_0_list = np.linspace(0, 2500, 40) 
        result_2 = {}
    ...
    if INITAL_PRESSURE:
        ...
        pressure_0_list = np.linspace(10.4, 14.4, 20)
        result_3 = {}
        # multiprocessing
