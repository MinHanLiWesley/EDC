...
def f(T_list=None,sc = None):
    ...
    head_flux = [sc*i for i in head_flux]
    hf_delta = mean_absolute_error(head_flux,head_flux_mine)

    loss = abs((MlX[-1]-55)*15 +hf_delta/300)
    return loss, MlX
...
def mainf(params):
    ...
    for _,value in params.items():
        T_list.append(round((T_list[-1]+value),2))
    loss, MlX = f(T_list=T_list,sc =sc)
...
if __name__ == '__main__':
    ...
    fspace={'t01' : hp.uniform('t01', low=10, high=20),
            't02' : hp.uniform('t02', low=10, high=20),
            't03' : hp.uniform('t03', low=8, high=15),
            't04' : hp.uniform('t04', low=5, high=15),
            't05' : hp.uniform('t05', low=5, high=15),
            't06' : hp.uniform('t06', low=5, high=10),
            't07' : hp.uniform('t07', low=5, high=10),
            't08' : hp.uniform('t08', low=0, high=10),
            't09' : hp.uniform('t09', low=0, high=10),
            ...
            't18' : hp.uniform('t18', low=0, high=2),
            't19' : hp.uniform('t19', low=0, high=2),
            't20' : hp.uniform('t20', low=0, high=2),
            't21' : hp.uniform('t21', low=0, high=2),
            't22' : hp.uniform('t22', low=0, high=2),
            'scale': hp.uniform('scale', low=0.9, high=1.2)
            }
    try:
        index_plt = 0
