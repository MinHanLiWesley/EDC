...
def f(T_list=None):
    ...
    head_flux = [i * total_heat_flux/15.94 for i in ratio]
    hf_delta = mean_absolute_error(head_flux[:12], head_flux_mine[:12])
    print(f"hf_delta: {hf_delta}")
    loss = abs(abs(MlX[-1]-target_X)+hf_delta/1000)
    if any(x>4 for x in X_delta):
        loss += 1000
    if T_list[-1] > 480:
        loss += 10000
...
def mainf(params):
    ...
    T_list = np.round(list(cs(range(23))),2)
    loss, MlX = f(T_list=T_list)
    print(f"T_list:{list(T_list)}")
    print(f"T_end: {T_list[-1]}")
    print(f"loss:{loss}")
...
if __name__ == '__main__':
    ...
    loss_dic = []
    try:
        os.mkdir(f"/home/wesley/EDC/FPC/ML/HyperOpt/0524/{NAME}")
    except:
        pass
    fspace = {'t00': hp.uniform('t00', low=50, high=80),
              't04': hp.uniform('t04', low=10, high=55),
              't07': hp.uniform('t07', low=5, high=30),
    ...
    try:
        index_plt = 0
        trials = pk.load(open(f"{NAME}/{NAME}_trials.pk", 'rb'))
    # 初始的數值
    ...
    for i in range(1, max_evals+1, step):
        ...
        pk.dump(trials, open(f"{NAME}/{NAME}_trials.pk", "wb"))
# head_flux = [18796.31, 21664.45, 22274.44, 23715.35, 24412.25, 24762.11, 25061.39, 24661.78, 23877.03, 21651.58,
#              20088.86, 18665.34, 17595.93, 16724.30, 15850.58, 15106.89, 14309.98, 13538.81, 12903.87, 12209.08, 11632.12, 11115.07]
