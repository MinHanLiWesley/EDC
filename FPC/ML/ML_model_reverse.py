...
def build_model(lr=0.001):
    ...
    second_input = Input(shape=(6,), name='Input_layer_2')
...
def DataPreprocessor():
    ...
    if DOKFOLD:
        ...
        X_train = [rescaled_X_train[:,2:-4],np.concatenate((rescaled_X_train[:,:2],rescaled_X_train[:,-4:]),axis=1)]
        X_test = [rescaled_X_test[:,2:-4],np.concatenate((rescaled_X_test[:,:2],rescaled_X_test[:,-4:]),axis=1)]
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, mass_flow_rate, 
    ...
    for i, T in enumerate(T_list[1:]):
        ...
        x_predict = [rescaled_x_predict[:,2:-4],np.concatenate((rescaled_x_predict[:,:2],rescaled_x_predict[:,-4:]),axis=1)]
