...
def main():
    ...
    for mass_flow_rate in np.linspace(72, 44, 10):
        for pressure_0 in np.linspace(11, 13, 5):   
            print('Training data creation initiated at {0}'.format(time.asctime()))
            f = open(f'Data/training_dataV5_C2H3Cl3//training_dataV5_C2H3Cl3_{End}.csv' , 'a')
            if os.stat(f'Data/training_dataV5_C2H3Cl3//training_dataV5_C2H3Cl3_{End}.csv').st_size ==0:
                f.write('Ti,Te,X1,X2,X3,X4,X5,X6,X7,X8,X9,X10,X11,X12,X13,X14,X15,X16,X17,X18,X19,X20,X21,X22,X23,X24,X25,X26,X27,X28,X29,pressure_0,CCl4_X_0,t,tr,Formation\n')
            for i in range(10):
                ...
                while not success:
                    ...
                    for j in range(n_pfr):
                        ...
                        information += str(forming_rates[j+1]) + '\n'
