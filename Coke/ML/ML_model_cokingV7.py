...
def build_model(lr=0.001):
    ...
    layer = Activation('relu')(layer)
    layer = Dense(6, name='Hinden_layer_5')(layer)
    layer = Activation('relu')(layer)
    ...
    output = Lambda(scale_out)([scale_input, output])
...
def DataPreprocessor(scale=20):
    ...
    if DOKFOLD:
        ...
        scaling_factor_train = np.full((len(y_train), 1), 0.101)
        scaling_factor_test = np.full((len(y_test), 1), 0.101)
        if scale == 30:
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, CHCl3_X_0, Tri_X_0, CP_X_0,
    ...
    y_predicted = [0]
    prev_y = 0
    scale_diameter = 0.202/2
    if len(T_list) == 23:
        scale_diameter = 0.262/2
