...
def build_model(lr=0.001):
    ...
    raw_third_input = Input(shape=(1,), name='Prev_coking')
    ...
    layer = Activation('relu')(layer)
    third_input = Lambda(lambda x: x / 25)(raw_third_input)
    layer = concatenate([layer, third_input], name='Concatenate_layer_2')
    layer = Dense(1, name='Hinden_layer_6')(layer)
    layer = Activation('sigmoid')(layer)
    output = Lambda(lambda x: x * 25)(layer)
    model = Model(inputs=[first_input, second_input, raw_third_input],
                  outputs=output)
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, CHCl3_X_0, Tri_X_0, CP_X_0,
            mass_flow_rate, n_steps, n_pfr, length, area, save_fig=False, name='predict', fold_no=None, iter_CCl4=False, FPC=True, print_cracking=False, loss=False, single=False):
    """
    ...
    for i, T in enumerate(T_list[1:]):
        ...
        x_predict = [Ti, Te, compositions,
                     pressure_0, CCl4_X_0,CHCl3_X_0,Tri_X_0,CP_X_0, t, t_r, prev_y]
    ...
    if DOKFOLD:
        ...
        if PREDICT:
            ...
            if single:
                ...
                for CCl4_X_0 in np.linspace(0, 2500, 11):
                    loss = predict(reaction_mech, T_list, pressure_0, CCl4_X_0,CHCl3_X_0,Tri_X_0,CP_X_0, mass_flow_rate,
