...
def arg_parser():
    ...
    parser.add_argument('--batch', default=1800, type=int, help='Batch size')
...
def scale_in(params):
    ...
    return pi * m.subtract(m.multiply(2*params[0],params[1]/1000),m.square(params[1]/1000))


def scale_up(params):
    return Multiply()([params[0]*10, tf.math.reciprocal_no_nan(params[1])])
def scale_in2(params):
    return pi * m. subtract(params[0],params[1])
def scale_out2(params):
    return 1000 * m.multiply(params[0],params[1])
...
def build_model(lr=0.001):
    ...
    layer = Activation('relu')(layer)
    third_input = Multiply()(
        [raw_third_input*(1/10), scale_input])
    # third_input = Lambda(lambda x: x / (25/(scale_input))(raw_third_input))
    layer = concatenate([layer, third_input], name='Concatenate_layer_2')
    layer = Dense(1, name='Hinden_layer_6')(layer)
    layer = Activation('sigmoid')(layer)
    # output = Multiply()([layer*10,1/scale_input,1/scale_input])
    output = Lambda(scale_up)([layer, scale_input])
...
def DataPreprocessor(scale=20):
    ...
    if DOKFOLD:
        ...
        scaling_factor_train = np.full((len(y_train), 1), 0.202)
        scaling_factor_test = np.full((len(y_test), 1), 0.202)
        if scale == 30:
...
def train_model(X_train=None, X_test=None, y_train=None, y_test=None, fold_no=None, verbose=TRAINING_VERBOSE):
    ...
    print(a)
    x_predict = np.hstack([X_train[0][1000],X_train[1][1000],a,X_train[3][1000]]).reshape(1, -1)
    print(model.predict([x_predict[:,:2],x_predict[:,2:-2],x_predict[:,-2],x_predict[:,-1]]))
    print("truth:")
    print(y_train[1000])
...
def predict(reaction_mech, T_list, pressure_0, CCl4_X_0, CHCl3_X_0, Tri_X_0, CP_X_0,
    ...
    y_predicted = [0]
    prev_y = 0
    scale_diameter=0.202
    if len(T_list) == 23:
        scale_diameter = 0.262
    ...
    if save_fig:
        ...
        if DOKFOLD:
            ...
            plt.savefig(os.path.join(
                PLOTPATH, f"predict/{name}/CCl4_{CCl4_X_0}_CHCl3_{CHCl3_X_0}_Tri_{Tri_X_0}_CP_{CP_X_0}\
                    _mass_{mass_flow_rate}_temp_{T_list[0]}_{name}.png"))
